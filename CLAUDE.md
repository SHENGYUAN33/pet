...
- 第三個切片（Image-to-Video＋簡易 FastAPI/前端，含後續的表單化 UI 與背景任務）：
  ...
  - **版型覆蓋層（第十二個切片）**：`pipeline/overlay_renderer.py`——資訊欄、對話氣泡、置中大字、結尾行動卡。這是 §4「字幕/貼圖/特效」裡 drawtext 做不到的那一半。
    ...
    - **視覺升級（字型／陰影／傾斜／圖示）**：`center_quote` 用手寫感字型（`config.OVERLAY_FONT_HANDWRITTEN`，那是寵物在講話，排版體會變成「別人寫牠的旁白」），其餘用圓體（`OVERLAY_FONT_ROUND`）。**字型檔不進版控**（二進位＋各自的授權，跟 stickers 不放美術素材同一條規則），放 `storage/fonts/` 再指過去即可；預設值是這台機器真的有的檔案（`kaiu.ttf` 是手寫角色的**替代品不是正解**），讀不到就一路 fallback 到 `DRAWTEXT_FONT_FILE`，**缺字型只會少一個字體，不會少一支影片**。所有卡片都有陰影——那是**卡片自己的 alpha 模糊後壓暗**，所以會自動貼合圓角與氣泡尖角，不用另外維護一個形狀；對話氣泡會傾斜（`OVERLAY_BUBBLE_TILT`），**整層一起轉**（底板、尖角、文字），而且**角度是照鏡頭序號決定的不是隨機的**，否則續跑會產出一顆跟原本不一樣的鏡頭（跟 `BACKGROUND_SEED` 釘死同一個理由）。資訊欄每行前面有圖示（蛋糕/針筒/愛心/腳印，同樣用 Pillow 畫），**由行內的關鍵字決定**（那一行是模型寫的自由文字，沒有結構化欄位可讀），沒對到的行給中性的腳印——一份清單有些行縮排有些沒有看起來像壞掉。換行寬度會**扣掉圖示佔的位置**，所以長行不會繞到圖示底下。`_band()` 也把陰影與傾斜的外擴算進去，整層仍然不會壓到資訊卡或字幕。
    - 實測（job 3476 → 3628，一支原本完全沒有版面的影片）：加上 `center_quote` 後只有第 1 顆鏡頭多出 `scene_1_overlay.png`，其餘鏡頭畫面不變，邊框/暈影/資訊卡/貼圖/字幕/版面全部在同一次編碼裡合成。

  - **局部道具（第十三個切片，只到 provider＋pipeline 層）**：`pipeline/props.py` ＋ `ImageEditingProvider.add_prop()`——在寵物身上畫一件小東西（項圈），或在牠旁邊放一個（玩偶）。
    - **這是全專案唯一允許改到動物本身的編輯**。背景那兩種最後都會把寵物真實像素貼回去，所以「寵物不會被重畫」是絕對的；項圈必須坐在脖子上，就是刻意**不貼回**一部分。它為此付三個代價：一定帶 AI 揭露標示、**腳本永遠不能選**（`config.PROPS_ALLOW_SCRIPT` 預設 0）、區域由**看過照片的人**指定。
    - **區域不是從 bounding box 推算的**。原本的想法是「主體框的前 15~30% 當脖子」，那只在動物站著、頭朝上時成立；這個專案的照片有不少是**俯拍**或**貓翻肚躺著**的，那條帶會落在地板上。SAM3 回的是一張沒有部位資訊的輪廓，問不出脖子在哪。所以由人指定——跟 `replace` 被降級成人工專用是同一個結論。
    - **矩形本身不是遮罩**：`collar` 是矩形 ∩ 主體輪廓（畫不到旁邊的地板），`toy` 是矩形 − 主體輪廓（放在旁邊而不是穿過牠）。這就是 `on_subject` 的全部意義，也是為什麼這件事不能用單純的矩形 inpaint 做。
    - **ControlNet 不是保住貓臉的機制**——最後的 `ImageCompositeMasked` 才是，而且它本來就在。Canny 買到的是**道具貼合身體的真實邊緣**而不是浮在上面。用 Canny 而不是 depth/openpose：ComfyUI 的 `Canny` 是**核心節點**，不必裝 `comfyui_controlnet_aux`。模型是 `diffusers/controlnet-canny-sdxl-1.0` fp16（2.5GB，放 `vendor/comfyui/models/controlnet/`），`preflight_props()` 缺檔時會指名該下載什麼、放哪裡。
    - **調參經驗**：第一次實測 strength 0.65 ＋ `end_percent` 0.8，**完全沒有畫出項圈**——邊緣圖裡本來就沒有項圈，強的 ControlNet 等於在說「脖子維持原樣」，跟道具生成的目的相反。改成 **0.35 ／ 0.5** 之後就正常了（實測：元寶那張翻肚照長出了一條紅白針織項圈，臉部像素完全沒動）。另外 `PROPS_MIN_SUBJECT_COVERAGE` 第一版寫 `0.5` 以為是 0.5%，實際上 `mask_coverage()` 回的是比例，等於 50%，把一張 SAM3 明明抓到 27% 的照片擋掉了。
    - **刻意沒做「人手伸進畫面」那條**。畫面裡有人手在摸貓，等於宣稱這隻動物可以被陌生人觸摸，而 Profile 沒說過——`config.BACKGROUND_FORBIDDEN_TERMS` 禁掉 `person／human／hands` 就是為了這件事，在這裡生成一隻手是繞過 `pipeline/fact_check.py` 而不是新增功能。
    - **已接進 `render_script()`／`GenerationJob`／網頁（第十四個切片）**：
      - **揭露標示會自動附加**（`_disclosure_notice()`，抽成純函式所以測得到）。關鍵決定：**兩種揭露併排顯示而不是互相取代**——「部分畫面由 AI 創意生成」講的是場景是假的，**它沒有講動物身上多了一件牠沒穿過的東西**，挑一個等於丟掉另一個講的事實。所以 `PROPS_DISCLOSURE_TEXT`（「含 AI 道具裝飾」）跟背景那句是分開的，兩者都成立時用 `DISCLOSURE_JOINER` 串起來。位置沿用既有的安全帶，資訊卡本來就會為揭露標示讓開。
      - **落盤**：migration `aca0dd58a857` 新增 `GenerationJob.prop_specs`（JSONB）與 `SceneJob.props`（JSONB）。`prop_specs` 是 **scene_id → 道具清單的 mapping**，不是 `background_scenes` 那種 id 清單——因為**區域是某一張照片上的位置**，沒有「全片共用的道具設定」這種東西。`prop_specs_from_job()`／`prop_specs_to_job()` 是一對，三個呼叫點（job 列、QA 計數、重生繼承）共用，不讓它們對「這次在畫什麼」有不同意見。
      - **存 region 不存 point**：point 要配合 placement 的預設尺寸才還原得出 region，而那個尺寸在 config 裡；存 point 的話**改一次 config 就會讓所有舊 job 的續跑畫在不一樣的範圍**。點是 UI 的輸入，region 是紀錄。
      - **修掉一個 bug**：單鏡頭重生原本只帶新放的那一個道具，等於**把前幾次加的項圈全部脫掉**。道具不像 overlay 那樣寫在 script 裡（它是審核者對某張照片的決定，記在 job 列上），所以要明確繼承。`regenerate_scene()` 現在會把來源 job 的 `prop_specs` 讀回來再蓋上新的那一顆。實測驗證：在有項圈的 job 上重生第 3 顆鏡頭加玩偶，第 1 顆的項圈仍在。
      - **網頁**：`GET /api/jobs/{job}/scenes/{scene}/source` 把 `visual_source` 解析成瀏覽器能顯示的 URL——放後端是因為那個對應**已經存在於 `pipeline/rendering.py`**，在 JS 再寫一份就是多一份要同步的。`photo:false` 是正常答案（影片／recap 鏡頭），UI 會說明原因而不是給一個按了沒用的控制項。介面是**點選不是拉框**：框的形狀本來就是設計決定（項圈 0.42×0.13、玩偶 0.26×0.20），點下去會畫出**伺服器實際會重繪的那個框**。
  ...
  - **版面自動避開寵物（第十五個切片）**：`pipeline/layout.py` ＋ `pipeline/subject_mask.py`。
    - **問題**：版型的位置本來是寫死的（資訊欄永遠右上、大字永遠正中），但**寵物在照片裡的位置每張都不一樣**。實測同一張翻肚照：資訊欄有 **76%** 壓在貓身上、氣泡 **79%**。舊的安全帶只避開了**其他文字**（上方資訊卡、下方字幕），完全沒有避開寵物——而寵物的輪廓其實一直都拿得到（背景置換和道具都在用），只是沒被用在這裡。
    - **做法**：每個版型提出**幾個願意坐的位置**，選寵物壓得最少的那一個。評分是對一張 24×42 的粗網格（`Occupancy`）算的，所以**核心邏輯不需要 GPU、不需要圖片就能測**——測試裡的遮罩是用 ASCII 手寫的（`#` 是貓）。實測結果：資訊欄 76%→**13%**、氣泡 79%→**0%**。
    - **候選順序就是設計偏好**：第一個是「沒有東西擋路時」會得到的位置，也就是舊的固定版面。`config.LAYOUT_MOVE_THRESHOLD`（0.08）讓**接近平手不算平手**——好 2% 但跑到對角的位置不是比較好，只是不一樣，而資訊欄在鏡頭之間跳來跳去看起來像壞掉。
    - **候選清單是設計決定不是自由變數**：行動卡只提供底部的位置（釘在上角的 CTA 是另一個更糟的設計，不是被救回來的設計）；置中大字**後來放寬到也能進空的角落**，因為參考設計就是把主標題放在照片的留白處而不是壓在動物上——所以那個名字現在比行為窄一點。
    - **元素之間也會互相讓開**：先放版面，把它**實際畫到的 alpha 範圍**（`painted_box()`，問成品 PNG 而不是另外追蹤，因為版面自己選位置、只有它的 alpha 知道最後落在哪）標成已佔用，貼圖再對剩下的空間挑。
    - **遮罩取得（`subject_mask.py`）**：`ImageEditingProvider.subject_mask()` 新方法（跟 `add_prop` 一樣是具體方法不是抽象方法，保持向下相容）。**照內容雜湊快取**在 `storage/decor/masks/`——同一張照片在續跑、單鏡頭重生、多個鏡頭重用時只會切一次。實測 **冷 4 秒、熱 2 秒**，所以每個照片鏡頭都問得起。
    - **失敗一律降級不失敗**：ComfyUI 沒開、切不出東西、provider 不支援——全部回空網格，於是每個候選同分、設計的第一選擇勝出，也就是**舊的固定版面**。缺遮罩只會少掉這個改善，不會少一支影片。`config.LAYOUT_AVOID_SUBJECT` 可以整個關掉。
    - 順手修掉一個**因為這個改動才浮現的**舊誤差：陰影外擴原本估 `blur*2`，實際 Gaussian 的支撐是 `blur*3`。以前版面都放在安全帶中間、看不出來；現在會貼著安全帶邊緣放，就多出去 4px 了。
