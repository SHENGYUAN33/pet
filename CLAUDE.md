...
- 第三個切片（Image-to-Video＋簡易 FastAPI/前端，含後續的表單化 UI 與背景任務）：
  ...
  - **AI 背景生成（第四個切片）**：這是「讓影片有場景感、不只是照片配模糊邊」那條路的第一大步，分兩種處理方式，差別是編輯上的而不只是視覺上的：
    ...
    - **replace（置換）**：把寵物去背，整個場景重新生成，可以把牠放到牠從沒去過的地方。**動物是真的，地點是假的**，所以這是策略 C，鏡頭會被燒上「部分畫面由 AI 創意生成」的揭露標示。
    ...
    - 走**跟 Wan2.2 同一台 ComfyUI 伺服器**，幾乎全是**核心節點**（`ImagePadForOutpaint` → `RemoveBackground`(BiRefNet) → `ThresholdMask` → `VAEEncodeForInpaint` → `KSampler` → `ImageCompositeMasked`），只多用 KJNodes 的 `GrowMaskWithBlur`（Wan 那條路已經需要 KJNodes）。要裝的只有兩個模型檔：SDXL checkpoint 放 `vendor/comfyui/models/checkpoints/`（`config.BACKGROUND_MODEL_FILE`），BiRefNet 放 `vendor/comfyui/models/background_removal/`（`config.BACKGROUND_MATTE_MODEL_FILE`，Hugging Face 的 `Comfy-Org/BiRefNet`，ComfyUI 原生支援）。缺哪個 `preflight()` 都會在開跑前講清楚檔案該放哪。
    ...
    - **replace 的已知限制**：（1）原本躺在床上/架子上的寵物換到草地後會「浮」在半空，因為姿勢與新地面的透視對不上，也沒有接觸陰影——用**模糊/淺景深的場景**明顯比有清楚地面的場景安全；（2）BiRefNet 找的是「顯著物體」，會把寵物趴著的貓跳台一起算進去，糊掉的照片還會留下方塊狀殘影。真正的解法是換成**帶文字提示的分割**（ComfyUI 已有 `SAM3_Detect`，用「cat」當提示只框動物），列為下一步。
