...
def stub_render(monkeypatch, tmp_path):
    ...
    def fake_build_scene_clip(*, visual_path, duration, subtitle_text, output_path):
...
def test_a_scene_that_fails_is_recorded_before_the_error_propagates(
    ...
    def explode_on_scene_2(*, visual_path, duration, subtitle_text, output_path):
...
def test_resume_reuses_finished_clips_and_only_renders_what_is_left(
    ...
    def explode_on_scene_3(*, visual_path, duration, subtitle_text, output_path):
        scene_id = int(Path(output_path).stem.split("_")[1])
        if scene_id == 3:
    ...
    def working_build(*, visual_path, duration, subtitle_text, output_path):
