...
- `python`／`pip`／`pytest`：PoC pipeline 開發與測試常用指令
...
- `ffmpeg`／`ffprobe`：影片剪輯（PoC 影音製作層核心工具）
- `node`／`npm`／`pnpm`（僅版本查詢與 `run`/`install`/`ci`）：預留給 MVP 階段的審核 UI（React/Next.js）
- `ls`／`dir`／`pwd`：基本檔案系統查詢
...
- **`postgres`**：透過 `@modelcontextprotocol/server-postgres` 連線資料庫查 schema／下查詢。連線字串走 `${POSTGRES_CONNECTION_STRING}` 環境變數，**不得**改成直接寫死帳密。對應本專案 MVP 階段規劃導入的 PostgreSQL（見 [CLAUDE.md](../CLAUDE.md) 建議技術棧）。
- **`github`**：透過 `@modelcontextprotocol/server-github` 操作 GitHub（讀 Issue/PR 等）。Token 走 `${GITHUB_PERSONAL_ACCESS_TOKEN}` 環境變數。

⚠️ 這是**範本**，目前 PoC 階段尚未有正式資料庫／CI 流程，實際填入連線字串與 token 前，請先確認：
...
- **`/gen-video`**：執行 PoC pipeline 生成寵物領養影片，對應 `pipeline/run.py`。
...
編輯或寫入檔案後自動觸發，依副檔名執行對應工具：`.py` 檔跑 `ruff check --fix` + `ruff format`；`.ts/.tsx/.js/.jsx` 檔（目前專案尚未有 `package.json`，此段為 MVP 階段審核 UI 預留）跑 eslint/prettier（若存在的話）。詳細中文註解已寫在腳本內。
