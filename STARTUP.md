...
- **核准或退回一個版本**：卡片上的「✅ 核准發布」「⛔ 退回」。有「缺少揭露／說了資料裡沒有的事／背景會暗示不實資訊」任一項時**不能核准**，按下去會告訴你是哪一項；退回一定要寫原因，那是下一次生成唯一的線索
...
- 針對單一鏡頭重新生成：先點一個版本，再從鏡頭清單點要改的鏡頭（不用自己記 job id / scene id），可換素材/字幕/旁白，或選 AI 動態化（Image-to-Video）與 AI 背景（延伸／置換）
- **AI 背景平常不用設定**：腳本本來就會幫每個鏡頭決定背景怎麼處理並照故事線推進；表單裡的「AI 背景」摺疊區只有在你要**推翻腳本的決定**時才需要填（要開著 ComfyUI，而且 prompt 必須用英文）
